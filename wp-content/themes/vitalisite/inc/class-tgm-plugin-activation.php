	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			$this->strings = array(
				...
				'menu_title'                      => __( 'Installateur de plugins', 'tgmpa' ),
				/* translators: %s: plugin name. */
				'installing'                      => __( 'Installation en cours: %s', 'tgmpa' ),
				/* translators: %s: plugin name. */
				'updating'                        => __( 'Mise à jour en cours: %s', 'tgmpa' ),
				'oops'                            => __( 'Something went wrong with the plugin API.', 'tgmpa' ),
				/* translators: 1: plugin name(s). */
				'notice_can_install_required'     => _n_noop(
					'Ce thème requiert le plugin: %1$s.',
					'Ce thème requiert les plugins: %1$s.',
					'tgmpa'
				),
				/* translators: 1: plugin name(s). */
				'notice_can_install_recommended'  => _n_noop(
					'Ce thème recommande le plugin: %1$s.',
					'Ce thème recommande les plugins: %1$s.',
					'tgmpa'
				),
				/* translators: 1: plugin name(s). */
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_plugin_advise_type_text( $required ) {
			if ( true === $required ) {
				return __( 'Requis', 'tgmpa' );
			}

			return __( 'Recommandé', 'tgmpa' );
		}
		...
		protected function get_plugin_status_text( $slug ) {
			if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
				return __( 'Non Installé', 'tgmpa' );
			}

			if ( ! $this->tgmpa->is_plugin_active( $slug ) ) {
				$install_status = __( 'Installé mais pas activé', 'tgmpa' );
			} else {
				$install_status = __( 'Activé', 'tgmpa' );
			}

			$update_status = '';

			if ( $this->tgmpa->does_plugin_require_update( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = __( 'Mise à jour requise non disponible', 'tgmpa' );

			} elseif ( $this->tgmpa->does_plugin_require_update( $slug ) ) {
				$update_status = __( 'Mise à jour requise', 'tgmpa' );

			} elseif ( false !== $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = __( 'Mise à jour recommandée', 'tgmpa' );
			}
			...
			foreach ( $this->view_totals as $type => $count ) {
				...
				switch ( $type ) {
					case 'all':
						...
						$text = _nx( 'Tous <span class="count">(%s)</span>', 'Tous <span class="count">(%s)</span>', $count, 'plugins', 'tgmpa' );
						break;
					case 'install':
						/* translators: 1: number of plugins. */
						$text = _n( 'A installer <span class="count">(%s)</span>', 'A installer <span class="count">(%s)</span>', $count, 'tgmpa' );
						break;
					case 'update':
						/* translators: 1: number of plugins. */
						$text = _n( 'Mise à jour disponible <span class="count">(%s)</span>', 'Mise à jour disponible <span class="count">(%s)</span>', $count, 'tgmpa' );
						break;
					case 'activate':
						/* translators: 1: number of plugins. */
						$text = _n( 'A activer <span class="count">(%s)</span>', 'A activer <span class="count">(%s)</span>', $count, 'tgmpa' );
						break;
					default:
						$text = '';
						break;
		...
		public function column_version( $item ) {
			...
			if ( $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . __( 'Version installée:', 'tgmpa' ) . '</p>',
					$color,
					$installed
				);
			}
			...
			if ( ! empty( $item['available_version'] ) ) {
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . __( 'Version disponible:', 'tgmpa' ) . '</p>',
					$color,
		...
		protected function get_row_actions( $item ) {
			...
			if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$actions['install'] = __( 'Installer %2$s', 'tgmpa' );
			} else {
				// Display the 'Update' action link if an update is available and WP complies with plugin minimum.
				if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
					/* translators: %2$s: plugin name in screen reader markup */
					$actions['update'] = __( 'Mettre à jour %2$s', 'tgmpa' );
				}
